...
class MockMvcWithMockServiceTestNoDatabaseHit {
    ...
    public void postProductTestWithNoDb() throws Exception {
        mockMvc = MockMvcBuilders.webAppContextSetup(context).build();
        Vendor vendor = new Vendor(1L, "XYZ", "NEPAL KTM", "2092092090");
        Product p=new Product(4L, "Head Phone4", "Good and Nice1", LocalDate.of(2020, 9, 9), new BigDecimal(34), 23, vendor);
        om.registerModule(new JavaTimeModule());
        om.disable(SerializationFeature.WRITE_DATES_AS_TIMESTAMPS);
        String jsonString=om.writeValueAsString(p);
        //System.out.println(jsonString);
        when(repository.save(any(Product.class))).thenReturn(p);      // Mockito.when(commentRepo.save(Mockito.any(Comments.class))).thenReturn(comment);
